...
export function OnboardingChecklist({ data }: { data: OnboardingData }) {
  ...
  return (
    <Card className="mt-6 border-gold/30 bg-gradient-to-br from-bg-cream to-white">
      <div className="mb-4">
        <h2 className="text-lg font-semibold text-navy-dark">{t("title")}</h2>
      ...
      <div className="space-y-3">
        {steps.map((step) => (
          <Link
            key={step.key}
            href={step.href}
            className="flex items-center gap-3 rounded-lg border border-border bg-white p-3 transition-colors hover:border-gold/50"
          >
            {step.done ? (
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-medium ${step.done ? "text-text-muted line-through" : "text-navy-dark"}`}
              >
                {t(`${step.key}Title`)}
              </p>
              <p className="text-xs text-text-muted">{t(`${step.key}Desc`)}</p>
            </div>
