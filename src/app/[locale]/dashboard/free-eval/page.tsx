...
export default function FreeEvalPage() {
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-navy-dark">無料診断を申し込む</h1>
        ...
        <Card className="mt-6 border-l-4 border-l-gold">
          {existing.status === "COMPLETED" ? (
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-navy-dark">診断が完了しました</p>
                <p className="mt-0.5 text-sm text-text-muted">
                  レポートは「レポート」ページからダウンロードできます。
                </p>
                {existing.url && (
                  <p className="mt-1 text-xs text-text-muted">対象サイト: {existing.url}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              <div>
                <p className="font-semibold text-navy-dark">申し込みを受け付けられませんでした</p>
                <p className="mt-0.5 text-sm text-text-muted">
      ...
      {submitted && (
        <Card className="mt-6 border-l-4 border-l-green-500">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-navy-dark">申し込みを受け付けました！</p>
              <p className="mt-0.5 text-sm text-text-muted">
      ...
      {!existing && !submitted && (
        <form onSubmit={handleSubmit} className="mt-6 max-w-2xl space-y-4">
          <Card>
            <h2 className="text-sm font-semibold text-navy-dark mb-4">申し込み情報</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={profile?.email ?? ""}
                  disabled
                  className="w-full rounded-lg border border-border bg-bg-cream px-3 py-2 text-sm text-text-muted cursor-not-allowed"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-text-body mb-1">
                  使用技術 <span className="text-xs text-text-muted font-normal">（任意）</span>
                </label>
                <select
                  value={form.stack}
              ...
              <div>
                <label className="block text-sm font-medium text-text-body mb-1">
                  気になっていること <span className="text-xs text-text-muted font-normal">（任意）</span>
                </label>
                <textarea
                  value={form.message}
                  ...
                  rows={3}
                  className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm focus:border-gold focus:outline-none focus:ring-1 focus:ring-gold resize-none"
                />
              </div>
            </div>
          </Card>
